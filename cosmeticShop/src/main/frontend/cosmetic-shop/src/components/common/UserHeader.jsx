...
export default function UserHeader() {
    ...
    return (
        <header className="w-full bg-white shadow-md">
            ...
            {/* 둘째줄 */}
            <section className="border-b border-gray-400">
                <div className="container mx-auto px-6 py-4 relative flex items-center">
                    {/* 좌측: 로고 */}
                    ...
                    {/* 중앙: 검색창 (absolute 중앙 고정) */}
                    <div className="absolute left-1/2 transform -translate-x-1/2 w-[400px] h-[40px]">
                        <input
                            type="text"
                            placeholder="Search"
                            className="w-full h-full pl-4 pr-12 bg-zinc-100 rounded-[5px] text-lg text-neutral-700 placeholder:text-neutral-400 focus:outline-none"
                        />
                        <MagnifyingGlassIcon className="w-5 h-5 text-neutral-500 absolute right-4 top-1/2 transform -translate-y-1/2" />
                    </div>

                    {/* 우측: Hello + Logout (로그인 시), 또는 고정 빈 공간 */}
                    {user ? (
                        <div className="ml-auto flex items-center space-x-4 min-w-[200px] z-10">
                            <span className="text-gray-700">Hello, {user.username} !</span>
                        </div>
                    ) : (
                        <div className="ml-auto min-w-[200px] z-10" />
                    )}
                </div>
            </section>
            {/* 셋째줄 */}
