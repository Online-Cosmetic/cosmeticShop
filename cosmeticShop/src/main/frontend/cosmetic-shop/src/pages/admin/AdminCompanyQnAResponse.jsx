// src/pages/admin/AdminCompanyQnAResponse.jsx

import React, { useState, useEffect } from "react";
import { useParams, useNavigate } from "react-router-dom";
import { adminAPI } from "../../utils/customAxios";

export default function AdminCompanyQnAResponse() {
    const { id } = useParams();
    ...
    if (loading) {
        return (
            <div className="w-full max-w-[1262px] mx-auto p-4 flex justify-center items-center min-h-[400px]">
                <div className="text-xl text-gray-500">Loading Company Q&A details...</div>
            </div>
        );
    }
    ...
    if (!qna) {
        return (
            <div className="w-full max-w-[1262px] mx-auto p-4 flex justify-center items-center min-h-[400px]">
                <div className="text-xl text-red-500">기업 QnA를 찾을 수 없습니다.</div>
            </div>
        );
    }
        ...
        <div className="w-full max-w-[1262px] mx-auto p-4 flex flex-col gap-4">
            <div className="w-full px-20 py-12 bg-white border rounded-2xl shadow flex flex-col gap-6">
                ...
                <div className="flex justify-between items-center">
                    <h2 className="text-3xl font-bold text-neutral-800">기업 Q&A 답변 작성</h2>
                    <button
                        onClick={() => navigate("/admin/company-qna")}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="flex flex-col gap-2">
                            <div className="flex items-center gap-2">
                                <span className="text-gray-500 font-medium">기업명:</span>
                                <span className="text-neutral-800 font-semibold">{qna.companyName || '기업명'}</span>
                            </div>
                            <div className="flex items-center gap-2">
