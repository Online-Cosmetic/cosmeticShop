...
public class ProductService {
    ...
    /* 기존의 단순 이미지 URL DTO 반환 로직을 실제 이미지 데이터(Base64 문자열)로 변환하도록 수정 */
    public ProductImageDTO getProductImages(Long productId) {
        ...
        for (ProductImage productImage : product.getProductImages()) {
            String base64Image = fileStorageService.loadFileAsBase64(productImage.getImageUrl());
            imageItems.add(new ProductImageItemDTO(productImage.getId(), base64Image));
        }
