            ...
            <div className="w-full px-20 py-12 bg-white border rounded-2xl shadow flex flex-col gap-12">
                ...
                {isLoading ? (
                    ...
                    <>
                        {/* Table Header - 열 추가 */}
                        <div
                            className="grid grid-cols-10 bg-neutral-50 p-4 border-b border-neutral-300 font-extrabold text-sm text-neutral-800 rounded-t-2xl">
                            <div>주문ID</div>
                            <div>이미지</div>
                            <div>상품이름</div>
                            <div>주문수량</div>
                        ...
                        {paginatedOrders.length === 0 ? (
                            ...
                            paginatedOrders.map((order, i) => {
                                ...
                                return (
                                    <div key={i} className="grid grid-cols-10 items-center py-4 px-2 border-b border-gray-100 text-sm hover:bg-gray-50 transition-colors">
                                        ...
                                        {/* 수량 추가 */}
                                        <div className="text-neutral-800 font-medium">{order.orderItemDTO.quantity}개</div>
                                        {/* 가격 추가 */}
                                        <div className="text-neutral-800 font-semibold">₩{order.orderItemDTO.price.toLocaleString()}</div>
                                        <div className="text-gray-600 text-xs">{order.address}</div>
                                        <div className="text-neutral-800 font-medium text-xs">
                                            {new Date(order.orderDate).toLocaleDateString('ko-KR')}
                                        </div>
                                        <div className="text-neutral-800 font-semibold">{order.buyerName}</div>
                                        <div>
                                            <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold ${statusInfo.class}`}>
                                        ...
                                        <div className="flex gap-2 flex-wrap">
                                            {statusInfo.nextStatus && (
                                                <button
                                                    ...
                                                    className="inline-flex items-center gap-1.5 px-4 py-2 bg-blue-500 text-white text-sm font-medium rounded-lg hover:bg-blue-600 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors shadow-sm hover:shadow-md"
                                                >
                                                    {statusInfo.nextStatus === 'PROG' && <TruckIcon className="w-4 h-4" />}
                                                    {statusInfo.nextStatus === 'COMP' && <CheckCircleIcon className="w-4 h-4" />}
                                                    {statusInfo.nextText}
                                                </button>
                                            )}
                                            ...
                                            {(order.orderItemDTO.deliveryStatus === 'READY' || order.orderItemDTO.deliveryStatus === 'PROG') && (
                                                <button
                                                    ...
                                                    disabled={updateDeliveryStatus.isLoading}
                                                    className="inline-flex items-center gap-1.5 px-4 py-2 bg-red-500 text-white text-sm font-medium rounded-lg hover:bg-red-600 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors shadow-sm hover:shadow-md"
                                                >
